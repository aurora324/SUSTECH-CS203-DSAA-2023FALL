...
//            node insert=new node();
            while (left.pre != head && !left.pre.flag) {
                left = left.pre;
            }
            left = left.pre;

            while (right.next != head && !right.next.flag) {
                right = right.next;
            }
            right = right.next;
...
//            int f=0;
            if (left == head) {
                ...
                if ((left.val ^ delete1.val) < (right.val ^ delete1.val)) {
                    delete2 = right;

                } else if ((left.val ^ delete1.val) > (right.val ^ delete1.val)) {
                    delete2 = left;
            ...
            delete(heap, top);
            top--;
            node insert = new node((delete1.val ^ delete2.val) + 1);
            insert.index = delete1.index;
            insert(heap, top, insert);
            top++;
            insert.pre = left;
            left.next = insert;
            insert.next = right;
            ...
            nodes[insert.index] = insert;
//            for (int j = 1; j < nodes.length; j++) {
//                if(nodes[j].flag) System.out.print(nodes[j].val+" ");
//                else System.out.print("("+nodes[j].val+")"+" ");
//            }
//            System.out.println();
        }
        while (!heap[1].flag) {
